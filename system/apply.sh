...
# Unlock the login keyring with the greetd login password.
...
# Kernel command line: no NVMe here; single-OS, so no os-prober scan.
...
# Services: Docker on demand, no boot wait for the network, no Bluetooth
# (no adapter), bounded package cache, Nix daemon.
systemctl disable --now docker.service NetworkManager-wait-online.service bluetooth.service 2>/dev/null || true
systemctl enable --now docker.socket paccache.timer nix-daemon.socket
